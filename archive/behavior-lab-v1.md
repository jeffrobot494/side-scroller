...
Reference: `archive/smarter-enemy-ai.md` (the eight-item
ladder this plan tracks against) and `tech/enemyspec-llm.md`
...
- **Companions on the shared brain (done via the locomotor refactor).**
  `tech/locomotion.md` Slices L1–L3 are **built**: movement split into a
...
- Covered by `test/behavior-lab.test.mjs` (headless mount; scoreboard records
  ...
  Visuals are an eyeball check. Mockup: `tech/mockups/behavior-lab.html`.
...
*Designed in full: `tech/agent-navigation.md` (nav graph → sense keys →
`navigate` controller → Lab overlays → content, as slices N1–N5). The sketch
below is what that doc expands on.*
