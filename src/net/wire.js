...
function check(value, path, seen) {
  ...
  // A cycle is not data. Caught before the recursion, or the walk never ends.
  if (seen.has(value)) return `${path} is a cycle`;
  seen.add(value);

  if (Array.isArray(value)) {
    for (let i = 0; i < value.length; i++) {
      const bad = check(value[i], `${path}[${i}]`, seen);
      if (bad) return bad;
    }
    return null;
  }
  ...
  for (const k of Object.keys(value)) {
    ...
    const bad = check(value[k], `${path}.${k}`, seen);
    if (bad) return bad;
  }
  return null;
}
...
export function assertData(value, what = "payload") {
  const bad = check(value, what, new Set());
