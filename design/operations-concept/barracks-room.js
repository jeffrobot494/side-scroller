(() => {
  ...
  const paint='#b09468',light='#f3ddb0';
  ...
  function box(x,y,z,dx,dy,dz,color='#2c3d43'){const a=[x,y,z],b=[x+dx,y,z],c=[x+dx,y+dy,z],d=[x,y+dy,z],e=[x,y,z+dz],f=[x+dx,y,z+dz],g=[x+dx,y+dy,z+dz],j=[x,y+dy,z+dz];face([a,b,c,d],shade(color,.82),'#17262b');face([e,f,g,j],color,'#22343a');face([a,e,j,d],shade(color,.62),'#132126');face([b,f,g,c],shade(color,.78),'#1c2d33');face([d,c,g,j],shade(color,1.16),'#41606a')}
  ...
  function render(){if(!w)return;ctx.fillStyle='#060c13';ctx.fillRect(0,0,w,h);const neb=ctx.createRadialGradient(w*.7,h*.2,0,w*.7,h*.2,w*.6);neb.addColorStop(0,'#1d25463b');neb.addColorStop(1,'#07101900');ctx.fillStyle=neb;ctx.fillRect(0,0,w,h);stars.forEach((s,i)=>{ctx.fillStyle=i%5?'#8dbdbb55':'#bdd7e588';ctx.fillRect(Math.abs(s.x)*w,Math.abs(s.y)*h,1,1)});
    ...
    face([[-8,-.2,-5],[8,-.2,-5],[8,-.2,18],[-8,-.2,18]],'#0e181c');
    face([[-8,7,-5],[-8,7,18],[8,7,18],[8,7,-5]],'#18282e');
    face([[-8,-.2,-5],[-8,-.2,18],[-8,7,18],[-8,7,-5]],'#14232a');
    face([[8,-.2,-5],[8,7,-5],[8,7,18],[8,-.2,18]],'#1d3037');
    face([[-8,-.2,18],[8,-.2,18],[8,7,18],[-8,7,18]],'#14232a');
    face([[-8,-.2,-5],[8,-.2,-5],[8,1.1,-5],[-8,1.1,-5]],'#182930');
    face([[-8,4.7,-5],[8,4.7,-5],[8,7,-5],[-8,7,-5]],'#22383f');
    // Draw the room shell first; furnishings and floor markings sit inside it.
    queue.sort((a,b)=>b.z-a.z).forEach(p=>p.paint());queue=[];
    for(let x=-8;x<=8;x+=2)line([x,-.19,-5],[x,-.19,18],'#0a1417');
    for(let z=-5;z<=18;z+=2)line([-8,-.19,z],[8,-.19,z],'#0a1417');
    for(const x of [-8,-4,0,4,8])box(x-.07,1.1,-5,.14,3.6,.16);
    line([-8,1.14,-4.8],[8,1.14,-4.8],'#62838a',1.5);line([-8,4.68,-4.8],[8,4.68,-4.8],light,2.5);
    for(const x of [-7.95,7.95]){line([x,.08,-5],[x,.08,18],paint,1.4);line([x,6.9,-5],[x,6.9,18],light,2);for(let z=-4;z<18;z+=4)line([x,0,z],[x,7,z],'#1a2c33')}
    for(const x of [-4,4])line([x,6.98,-4],[x,6.98,16],light,2.5);
    // Paired bunks and personal lockers.
    for(let k=0;k<3;k++){const z=-3+k*2.8;box(-7.8,.3,z,2.2,.25,2.2);box(-7.65,.55,z+.1,1.9,.15,1.9,'#46554e');box(-7.8,2,z,2.2,.18,2.2);box(-7.65,2.18,z+.1,1.9,.13,1.9,'#46554e');line([-5.6,2.17,z],[-5.6,2.17,z+2.2],paint,1.4)}
    staff.slice(0,3).forEach(([id,name],i)=>{const x=-3.5+i*2.25,lit=active===id;box(x,0,-3.85,1.6,2.6,.8,lit?'#34474a':'#34474a');line([x,2.65,-2.99],[x+1.6,2.65,-2.99],lit?'#b0ffcf':paint,lit?3:1.5);text(name,[x+.8,.56,-2.98],lit?'#ceffe1':paint,10);line([x+.17,.2,-2.98],[x+1.42,.2,-2.98],paint,1)});
    // Metal benches, canvas equipment bags, and a painted floor stencil.
    box(-4,.2,2.7,3,.55,.8);box(2,.2,2.7,3,.55,.8);box(-3.7,.75,2.9,.65,.3,.4,'#6b563c');box(3.6,.75,2.9,.8,.25,.4,'#6b563c');
