...
| Server settings C1–C3 — `scope: "server"` names the 47 knobs only a room reads, `GET`/`POST /api/config` carry them, and `editor.html?server=1` tunes a running server (Reset stays local; Export and Import cross). C4–C5 (a "Make permanent" button writing the running values into the config schema in source) are specced, not built | `design/server-settings.md` · `tech/server-settings.md` |
