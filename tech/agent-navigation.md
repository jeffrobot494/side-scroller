...
| N3 | **Route following.** Body→envelope mapping, graph cache, routing in the `moveOrder` branch and the **point-resolving** controllers (`chase`, `moveTo`), explicit `driveX { hop }` in both locomotors, partial-path fallback, stuck detector, 3-attempt cap, navigation senses | **Changed.** Grounded agents on a resolved destination traverse terrain. `keepDistance` agents and all combat behaviour untouched |
