...
- **As built, S3 — three of the four things it names had already shipped.** Landing acceptance became a supported-position test the moment S2 widened the span, because the span IS the supported extent (`landsOn` reads `to.a`/`to.b` and needed no edit). The in-flight aim moved in S2 as `settleX`. The follower's commitment to a takeoff the predictor validated shipped in C2 as `nav.commit`. What was actually left is the takeoff SET, and that is what S3 is.
- **As built, S3 — the takeoff set gained a RUN-UP, and it only walks backwards.** Measured over 60 levels: of the hop/jump edges no ordinary takeoff can fly, 138 are flyable from somewhere else on the source surface, and **every one of those positions is further from the destination than the ordinary takeoff — not one is nearer.** What the distance buys is rise before arrival: a body flush against a block on its own floor jumps and drives at its landing point on the same frame, walking into the block's SIDE, where from a run-up it is above the block by the time it gets there. So the set is the ordinary takeoffs plus a ladder stepping away from the destination — two body widths, in thirds of one, bounded also by what the edge's reach budget has left after the gap. Two body widths recovers 134 of the 138 and 90px recovers one more; of what the built version still misses, 3 are beyond the ladder and 85 are the band below.
- **As built, S3 — the run-up is tried only when every ordinary takeoff fails.** Not an optimisation first: a body should not walk backwards to make a jump it can make from where the route already takes it, and the follower commits to whichever validated takeoff is nearest, so a far-back entry in the set of a working edge would pull it there. It is also what keeps the cost down — the extra flights are paid on the third of edges that were being rejected anyway.
- **As built, S3 — the takeoff band is two-sided.** C2 tested the takeoff plus one frame of travel on the side the body came from, on the reasoning that a body walks toward its takeoff and the takeoff is at the end of the span. A run-up takeoff is in the MIDDLE of the span, and is approached from the destination side by a body already standing at the lip. Both offsets are tested now, each clamped to the span and each required to clear the destination's footprint — filters that reject exactly one side for every ORDINARY takeoff, so the band is what it was: measured over 60 levels, **0 of 6,030 existing edges changed and 0 takeoffs moved.** It costs 85 of the run-up rescues, which is the trade stated in Approximations.
- **As built, S3 — measured, and it is thin.** Over 60 levels: **82 hop/jump edges added, 0 removed, 0 takeoffs moved**, and those 82 add **5 reachable (from, to) pairs in one seed** — everything else is a shortcut across terrain that already connected. The frozen reachable-surface guard does not move on any of its 12 seeds. The 60-level agent sweep (437 grounded agents, 20s each) is byte-identical before and after, down to the airborne frame count. Graph build goes 0.75ms → 1.06ms average. **S3 is correct and nearly free of consequence, and the reason is S2.**
- **As built, S3 — "70% of the rejected climbs are flyable" was true of the pre-S2 spans and is not true now.** Re-measured on the built graph, replaying every hop/jump edge clearance drops from every supported position at five launch velocities: over 60 levels **1,081 of 1,191 (91%) are not flyable from anywhere at any speed** — the predictor is simply right about them. 88 are flyable from a standing takeoff the run-up does not reach (85 of those refused by the two-sided band, 3 beyond the ladder), and **22 need a running launch, which is all S4 stands to gain on this side of the ledger** — S4's value is on the other side, the accepted edges a running body then fails. So **the reported "won't climb" was S2's to fix, and S2 fixed it.**
- **The seeds that still lose most of their surface lose it to geometry, not to the model.** Seeds 22, 55 and 121 are each cut in two by one obstacle, and replaying the frontier edge frame by frame shows why: the body clears the obstacle's top and then puts its HEAD into a shelf a little higher up, while still rising. It is standable underneath — the headroom cut correctly leaves the surface in the graph — but there is no arc into it from the floor, at any takeoff and any launch speed. The legacy graph offers the edge because it tests only where a jump lands. Nothing in S3, S4 or S5 will move those seeds, and the generator's audit uses the legacy model, so the level may well be cut in two for the player too. That is the "Generation keeps envelope reachability" row, met in the wild.
- **A rule S3 did NOT change, having measured it.** The predictor rejects a flight on ANY side contact, where `collideAxis` pushes the body out of the wall, zeroes its horizontal velocity and lets it keep rising — a body that scrapes a column and then lands is a jump the follower flies and completes. Making the predictor faithful there adds **56 edges and 0.1% of reachable pairs over 60 levels**, for another 0.2ms of build. Not worth breaking C2's integration rules for; the run-up already covers the shape that motivated it.
- **As built, S4 — the launch velocity was not the whole of it. The ACTUATION was.** The plan said the arcs start at the velocity a body carries in. Built that way alone it moves almost nothing: the predictor recomputes `vx` from the airborne aim every frame, so a launch velocity is a one-frame ±5.3px perturbation, and over 60 levels it costs 5 hop/jump edges and changes the failure rate not at all. What actually diverges is every frame after it. A legged body's `vx` IS its drive request; a SOLDIER reads the sign and accelerates at 2600px/s², so it needs six frames to stop moving the way it arrived. Traced frame by frame, the failing climb is a body that launches at 270px/s beside a ledge, drifts 13px under it while the aim tries to hold the footprint edge, and puts its head into the ledge's own underside. So `bodyProfile` carries `accel`/`friction`, `actuate()` is one frame of the body's horizontal response, and the predictor uses it at the launch AND at every step of the arc.
- **As built, S4 — the actuation is part of the graph's identity, and that is the point.** `profileKey` gains `+accel/friction`, so a 30x46 soldier and a 30x46 legged body no longer share a graph: they have the same envelope and cannot fly the same jumps. Two callers describe a soldier body, and `soldierProfile(w, h, gravity)` in `src/mission/navigation.js` is now the only one of them — `mission.js`'s squad-path overlay built its own and would have drawn a legged body's graph beside a squad routing on a soldier's. One definition rather than a test for the divergence.
- **As built, S4 — a takeoff carries a DIRECTION, which S3 predicted it would have to.** For a soldier, 96% of validated takeoffs are one-sided (5,188 of 5,429 over 60 levels; for a legged body it is 5%). The flank beside a ledge is clean launched away from the ledge and a head-bonk launched toward it, and both are the same x. Requiring a takeoff to fly from both approaches costs 581 of 1,988 hop/jump edges and 44% of reachable pairs; resolving the set one direction at a time costs 95 edges and no reachable surface at all. `link.takeoffs` is `[{ x, dirs }]`, and `takeoffFor` in `src/mission/navigation.js` commits only to a takeoff whose `dirs` include the side it will arrive from. That also pays back S3's two-sided band: on a legged profile, where nothing else about S4 bites, hop/jump edges go 1,988 -> **2,044**.
- **As built, S4 — the run-up is resolved per takeoff and per direction, not per edge.** S3 tried the ladder only when EVERY ordinary takeoff failed. That is wrong once takeoffs have directions: the far flank of a ledge flies for a body running right, so an edge-wide fallback stops there and leaves a body coming from the left to walk PAST the ledge and climb the other end — legal, and it reads as confusion. Laddering behind each ordinary takeoff that failed for THIS direction gives the near-side run-up instead. `runUpTakeoffs` became `runUpFrom(c, ...)`, one candidate's ladder, and it stops at the first rung that flies: the follower commits to whichever validated takeoff is nearest the body, so the remaining rungs are a choice nothing uses, at four flights each. Same edge counts, and the build cost of a soldier graph falls by about a third.
- **As built, S4 — the arrival SPEED is a range and both ends are required.** A follower can reach its takeoff at anything up to a run, so each direction is flown twice: at run speed, and from a standstill (an agent that repathed while already standing there). Dropping the standing end keeps 128 more edges and doubles the failed jumps that survive S4, 70 -> 129. The running end is tested first because it is the end that rejects.
- **As built, S4 — measured, and this one is not thin.** Every node of 60 generated levels gets a real `Soldier` sent to the far end, and every route LEG that ends anywhere but where its edge pointed is counted: **2,072 of 14,098 legs failed in the air before (14.7%), 70 of 12,003 after (0.58%)** — a rate the C2 sweep could not see, because it counted the ledger left at the end of a run rather than each resolution. 95 of 1,988 hop/jump edges are dropped on a soldier profile (and 56 gained on a legged one), reachable pairs fall 5.7%, and **the frozen reachable-surface guard does not move on any of its 12 seeds for either body** — which is the claim S4 has to make. A graph build costs about 1.8x more legged and 3x more for a soldier (1.6ms -> 2.6ms / 4.8ms average over 60 levels, worst ~7ms).
- **As built, S4 — what it does NOT fix is reaching a takeoff from the wrong side.** A body standing at a lip whose only validated takeoffs are behind it walks back to one and launches carrying the speed it walked back WITH, which is the opposite of the direction that was validated. The follower refuses a mismatched takeoff where it has an alternative and falls through to attempting one where it does not, exactly as `takeoffX` does when no clear side exists — so the cap still retires the edge and the reroute still happens. Approaching from the far side means walking PAST the takeoff and turning around, which is follower machinery, not predictor machinery, and it is what the residual 0.58% is. `test/navigation.test.mjs`'s S3 lip case is the named example.

- **As built, S5 — the reflex was not a residue. It was half the jumps.** The slice was priced at 8 of 310, from a sweep that sent agents the LENGTH of a level: a destination on the ground is never above anything, and `hopToward`'s whole trigger is "the target is above me". Re-measured with the destination on the highest node of each of 60 levels and an agent started on every other node, over 1,968 runs per body: a soldier body takes **3,663 blind hops against 2,859 routed ones**, a legged body 2,306 against 2,273. Standing off the graph with somewhere to be above you is the ordinary case, not a corner, because the pockets the graph deletes for headroom are exactly the places something is overhead.
- **As built, S5 — the decision is the caller's, and the soldier locomotor was making it.** `hopToward` is already the request field that means "try", and `LEGGED` honours it only on `driveX`; a `steer` has never hopped a legged body at all. But `SOLDIER`'s `steer` branch hopped on its own initiative off `req.point` — the one locomotor that jumped without being asked, and the path the Behavior Lab agent, every escorted squadmate and both `moveTo`/`moveOrder` fallbacks arrive through. It now reads `req.hopToward` like its own `driveX` branch does, and `reflexHop()` in `src/mission/enemyspec/runtime.js` is the only thing that sets it: the point for a body with no graph, `undefined` for one that has a graph.
- **As built, S5 — `navGraph(ent, scene, speed)` is the one test, and the existing consumers were rewritten onto it.** "Is this body routed at all" was spelled out twice already (`routeRequest` and `holdPoint` each opened with four guards), and the fallback sites needed it a third time. It is one exported function now and both consumers call it. It costs nothing at a fallback site — `routeRequest` has already built and cached the graph for that profile — **provided the caller passes the speed it routed with**, because a legged profile carries its run speed and a different one is a different graph.
- **As built, S5 — measured, and it costs 7 arrivals in 1,968.** Same sweep, per body: soldier blind hops **3,663 → 0**, arrivals 184 → 178, mean distance left to the goal 1,378 → 1,401px, and route legs failing in the air **117 → 100** (the hop was also a way to fail); legged blind hops **2,306 → 0**, arrivals 289 → 289, mean distance 1,622 → 1,624px. 422 soldier runs end further from the goal and 290 end nearer. The graph does not move — S5 changes no arithmetic in `src/game/nav.js` — so the frozen surface guard and the S4 failed-leg ceiling are untouched, and `test/mission.golden.json` did not move either: no companion in the golden trace ever took a blind hop.
- **As built, S5 — what the lost arrivals expose is a DROP edge, and the hop was masking it.** Six of the seven are one geometry, seed 48: the route out of node 17 (a 120px shelf at y=287) is `17 -drop-> 13`, and node 13 is 110px further left and only 39px lower, so a body that walks off the lip at run speed is past 39px of fall long before it is over the shelf, and lands on the floor two nodes below. Before S5 that step-off was rescued one frame later by the reflex, in the coyote window: the hop turned the drop into a jump and put the body on node 14 — the goal — a level ABOVE what the route had planned. No falling trajectory is validated ("Walks and drops are unchanged"), so this is the drop model, and removing the blind hop makes it visible rather than causing it.
...
|---|---|
| Ordinary takeoff styles, not every physically possible jump | The predictor tests takeoffs a follower will actually use: the positions beside the destination, and — for each of those that does not fly in a given approach direction — a run-up of up to two body widths away from it, in thirds of one, bounded by the edge's remaining reach budget, stopping at the first rung that flies. A human's earlier jump or unusual steering may cross terrain the agent declines. As built the bound is not what binds: 3 of the 88 rescues S3 could not reach are out of the ladder, and the rest were the two-sided band S4 removed |
| ~~A takeoff must work from either side of itself~~ | **Removed by S4**, which is what it said the fix would be. The set is resolved one arrival direction at a time and each takeoff records the directions it flies from, so the band is one-sided again — the takeoff plus one frame of travel behind it. It gave back 56 of the 85 edges on a legged profile |
...
**This table describes the graph C2 shipped, and S2 replaced it.** Re-measured on the built S0–S3 graph the ratio inverts — 91% of what clearance drops is not flyable from anywhere at any speed — because the first contributor below was most of the 70% and S2 is what removed it. See the S3 As-built notes above.
