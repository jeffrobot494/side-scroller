...
|---|---|
| Ordinary takeoff styles, not every physically possible jump | The predictor tests takeoffs a follower will actually use: the positions beside the destination, and — only when none of those flies — a run-up of up to two body widths away from it, in thirds of one, bounded by the edge's remaining reach budget. A human's earlier jump or unusual steering may cross terrain the agent declines. As built the bound is not what binds: 3 of the 88 remaining rescues are out of the ladder's reach and 85 are the row below |
| A takeoff must work from either side of itself | The follower commits on the first frame inside its window and can arrive at a mid-span run-up takeoff from either direction, so the predictor validates one frame of travel to both sides and keeps the takeoff only if both fly. A takeoff that works from one approach and not the other is refused rather than gambled on. Measured at 85 edges over 60 levels, against 82 kept — the single largest thing standing between the run-up and the edges it could rescue. Recording which side a takeoff is good from, and refusing to commit from the other, is the fix, and it needs the follower's arrival direction, which is S4's subject |
