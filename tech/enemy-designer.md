...
| | |
|---|---|
| **The admission gate lives in the store, not the merge** | The plan put `rosterspecs.js` down as "the roster store **only**". `admitSpec()` — declared threat, pinned id, `accept()` at `ROSTER_DRYRUN_SECONDS` (12) — went there anyway, next to the thing it guards, which keeps `enemyspecs.js` (production content) free of the acceptance pipeline. What did NOT go there is the never-empty policy, because that spans built-ins the store cannot see |
...
| `src/game/enemystore.js` (new) | **E6.** The delta store: edits by id, tombstones, additions, in-missions flags, guarded storage, and a one-shot read of E4's two keys so nothing already authored is lost. Imports nothing from `enemyspecs.js` — the reserved-ids argument goes away with the reserved ids, which is what makes the import one-way without a snapshot |
| `src/game/rosterspecs.js` | **E6: deleted.** Its admission gate, enable flags and never-empty rule move into the store and the merge. **E4:** The roster store — guarded storage, membership, enable flags. Imports nothing from `src/game/enemyspecs.js`. **As built (E4):** the admission gate (`admitSpec`, `ROSTER_DRYRUN_SECONDS`) shipped here too, so it sits next to the store it guards; the never-empty policy did not, because it spans built-ins |
...
| `test/content.test.mjs`, `test/tools.test.mjs`, `test/gen.test.mjs` | The three suites that import `rosterspecs.js` directly. E6 deletes that module, so their roster sections move to the new store rather than being dropped — the assertions about admission, never-empty and the merge reaching generation all still have to hold |
...
| `test/docs.test.mjs` | This document's citations. **A hazard E6 must handle in its own commit:** deleting `src/game/rosterspecs.js` while this document still backtick-cites it fails the suite — a cited `src/` path must exist unless it is marked `(new)`. The citation is rewritten in the commit that deletes the module, not after |
...
| 5 | **Roster admission dry-runs longer, not more realistically.** `dryRunSpec` builds a single flat platform and one stationary dummy; raising `seconds` does not add terrain, a squad, or a mission's length | Engine-clamped `limits`, and `loadMission` falling back on an unresolvable id — **as built that fallback is the hardcoded `missionSpecById.husk_charger` (`src/mission/entities.js`), not "the cheapest", and E6 has to make it true because `husk_charger` becomes deletable (see the seam).** Making the arena realistic means editing `src/game/enemyspec/dryrun.js`, which this spec does not own |
