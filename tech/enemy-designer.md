...
| 6 | **As built (E4): `ROLE_BEHAVIOR` in `enemyspecs.js` is the derivation.** **A custom enemy's placement hint is derived from `role`, and the derivation is lossy.** The built-ins prove role does not determine it — `cowardly_duelist` and `sky_duelist` are both `elite` and get different hints | Eyeball in Tools → Level Generator, which previews placements. Built-ins keep their authored hints |
