...
| **Dealing the recruit pool is a rule this spec invented.** `design/multiplayer.md` says nothing about recruits. S3's "each recruit goes to exactly one player, so hiring is scarce without being contested" is a design decision, not an implementation of one, and it is flagged here rather than buried because `CLAUDE.md` puts design out of this document's hands. It also does not scale: `RECRUIT_POOL` holds six authored soldiers, so dealing gives three each at two players, two each at three, one each at six | **Open, for Bo.** The build does not need the answer until S3. The ways out — dealing, a larger authored pool, recruits arriving over time, per-player copies with namespaced ids, or a generator — differ in what they do to hiring, not in what they cost to build |
...
| **The board ceiling is shared, and was tuned for one player.** `config.leadCount` is a ceiling and `leadArrivalRate` an arrival rate for a board two commanders now draw from, so each sees roughly half the work a solo campaign offers, before S6 narrows visibility further. `tech/campaign-pacing.md`'s ceiling and thin-board rule are unchanged in code and changed in effect | Nothing here. Both are config knobs, so the answer is playtesting rather than a slice — but whether a shared board should be the same size as a solo one is a design question, not a tuning accident |
