...
| `test/session.test.mjs` — **three exact-list assertions on the public surface** | The suite pins the session's method list and the view's field list by exact equality, and asserts `highWins` is absent. S4 grows both lists — a readiness surface on the session, other commanders' readiness on the view. Exact lists are the right shape and should stay exact; they just have to be updated deliberately rather than discovered by a red bar |
...
| `node test/run.mjs` | 31 suites, 1336 assertions green before S1 begins; 32 suites, 1379 after it; 32 suites, 1405 after S2 and its fix; 32 suites, 1419 after S3. **Run it more than once** — the ambient suite is randomised, and the case S2 first shipped failed about two runs in five while reporting green on the others |
...
| **Where the day sits relative to a mission is the player's choice at S4.** The gate spends the day whenever it is pressed, so a commander may deploy before or after turning it. `design/multiplayer.md` fixes the order — ready, then both missions begin — but there is nothing to lock until S5 holds a pending choice, so S4 cannot enforce it | S5, which locks choices and turns the day on one click and is where "before any mission runs" becomes real. The consequence to watch for meanwhile: today a win's `threatReward` is banked before the same call's doom tick, so a win can outrun the clock on the last day. After S4 the two are in different clicks, and after S5 the doom tick comes first |
