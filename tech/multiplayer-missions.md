...
| 11b | **A finished commander leaves, and the mission they leave is still running — so a result is reported while the scene it describes goes on changing.** Bo: *"Commanders who finish their mission should return to base, not wait for the last player to finish."* Their `_resolve` is taken at the moment they extract, which is correct — but their soldiers stay in `scene.soldiers` as departed, so anything that later reads the scene for that owner reads a squad that has gone home | `test/mission-ownership.test.mjs`: a result taken at extraction does not change when the scene runs on. The trap is any J8 snapshot or result path that re-derives an owner's state from the live scene rather than from the result taken when they left |
