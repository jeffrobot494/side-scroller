...
| **J6** | **Built. The mission runs without a browser.** `Mission.start()` calls `this.input.enable(canvas)` and `requestAnimationFrame`, both unguarded, so in bare node it throws `window is not defined` — the golden only works because `test/run.mjs` calls `installDom()` globally before any suite. A host-free construction path: no DOM globals, no rAF loop, input injected rather than device-bound. Verified by a suite that constructs a Mission **with the harness deliberately not installed**. **As built: the canvas IS the flag** — `new Mission(null, onComplete)` is the host-free path, no option bag and no factory, because the only three things a canvas decides are the device, the loop and the drawing. See "What a host actually is" | No. The browser path is unchanged; this is the door the server needs |
| **J7** | **Built. Input goes per owner.** One `Mission` today has one input and one piloted soldier: `_updateSoldiers` pilots `leaders.get(this.owner)` alone, `_handleControl` reads `justPressed("swap")` for `this.owner` alone, and `_applyAim` resolves a mouse position against `this.camera` and `this._zoom()` — one viewer's frame. Stepping a scene off two seats needs an input per owner, a swap per owner, and aim that arrives already in world coordinates. **The largest mission-side change in the phase, and the previous draft assigned it to nobody**. **As built it was not the largest: ~60 lines in `src/mission/mission.js` and NONE in `src/mission/input.js`** — the three sites were already partitioned by owner for everything except which input they read. See "Three shapes of aim, and one map" | No at one owner. It is what makes two possible |
| **J8** | **Built. The room holds the mission and broadcasts it.** `server.mjs` steps a headless `Mission` at a fixed 60Hz off each seat's latest input and sends each seat **its own** snapshot; the page sends input at a fixed rate and draws what comes back. No prediction, no interpolation, no local simulation. Both `missionResult` commands are sent by the room, one per dispatch id, because the room is what holds the results. **A seat that resolves goes back to base and stops receiving snapshots; the room keeps stepping for whoever is left**, which is the design's rule and is only possible once the simulation is not the leaving player's page. **As built: two channels split by cadence, and a `hosted` flag is the only thing the page forks on.** See "What a snapshot is, and what it is not" | Yes. The design's decision 5, and the first time anybody plays *with* somebody |
...
input decoupled from a frame rate for the same reason a peer did. **Every slice
is built.** J8 needed nothing invented in it: the owner axis, independent ends,
two results for one lead, fixed-step input, a joint dispatch, a host-free
construction path and an input per commander were all in before it started, and
what it added was a socket, a snapshot and a loop. **Nothing played jointly until
J8**, and every slice before it landed with no transport at all — which is why
the risky half was three files rather than the whole phase.
...
**As built, that count was low: there are FOUR host contacts, not two**, and the
two the paragraph names are the middle of the list. Measured by peeling them off
one at a time in a bare process:

| Order | Contact | Where |
|---|---|---|
| 1 | `canvas.getContext("2d")` | The **constructor**, before `start()` is reached at all — so the first thing in the way is not a global but the argument |
| 2 | `window.addEventListener` | `start()` → `this.input.enable(canvas)` |
| 3 | `requestAnimationFrame` | `start()`, last line |
| 4 | `window.removeEventListener` | **`stop()`** → `this.input.disable()`, which no draft mentioned. A mission that starts host-free and cannot be stopped host-free is not a door the room can use |

**And `performance.now()` is not on the list**, which is worth saying because it
reads like it should be: node has `performance` as a global, so the one call
that looks browser-shaped is the one that was never a problem. What was left
after the four: `update()` runs untouched in bare node, including
`_updateCamera` — see the viewport note below.
...
which is also, exactly, the shape approximation 6 describes for a commander who walks away. **J7 is built**: any commander may now have an input, and one with none falls to that brain by being absent from a map rather than by a branch |
...
## What a host actually is (J6 — built)

The slice asked for "a host-free construction path". As built there is no
second path: **the canvas is the host, and it is optional.**
`new Mission(canvas, onComplete)` is the browser's mission and
`new Mission(null, onComplete)` is the room's, and the difference is exactly
three things.

| What a canvas decides | Host-free |
|---|---|
| The **device** — `input.enable(canvas)` binds the keyboard, the pad and the mouse | Not called. The mission holds a `MissionInput` it never enables |
| The **loop** — `performance.now()` + `requestAnimationFrame(this._frame)` | Not armed. Whoever constructed it owns the step |
| The **drawing** — `getContext("2d")` | `this.ctx` is `null` and `render()` returns immediately |

Nothing else moved, and that is the claim worth guarding: `update()` is one
function with no host in it, and the golden asserts a canvas-less mission plays
the mission a canvas-ful one plays, step for step.

**Four things the plan did not say:**

| | |
|---|---|
| **A host-free mission still needs a VIEWPORT** | `_updateCamera` is the last call in `update()` and reads `canvas.width`/`height` every step, so "no canvas" cannot mean "no dimensions". It gets a plain object, which `_applyCanvasSize` then sizes from `config.missionCanvas` exactly as it sizes a real canvas — so both paths solve the same camera from the same preset rather than one of them solving a different one. The room will hold one viewport for two viewers, which is approximation 5's problem and not this slice's |
| **`stop()` is the fourth host contact, and it is on the input** | `input.disable()` removed window listeners unconditionally, so a mission that could start host-free still could not end. `disable()` now returns early when `_enabled` is false — the mirror of the guard `enable()` already had. Symmetry, not a special case: an input that never bound a device does not need one to let go |
| **`MissionInput` gained `reset()`** | `enable()` did two jobs — bind the device, and reset the per-mission state (`frame`, the latched sample, the pad diagnostics). Host-free wants the second without the first, so the second is now its own verb and `enable()` calls it. This keeps J4's invariant true on both paths: `frame` is the step index, **numbered from a mission's start**, and a room reusing one input across two missions would otherwise keep counting. `test/mission-trace.mjs`'s stand-in carries `reset()` for the same reason it already carried `enable`/`disable` |
| **Input is not "injected" yet, and does not have to be** | The slice row says "input injected rather than device-bound". As built a host-free mission reads blank frames off its unenabled `MissionInput`, so it runs with *no* input rather than requiring one. That is the correct floor: J7 is what gives a mission somewhere to inject input per owner, and J6 should not invent a narrower seam that J7 then replaces. **Corrected while building J7: this row said "nobody is piloted", and that is wrong.** `this.owner` is the first soldier's owner and its leader IS piloted — off a blank input, so it stands still rather than being AI-driven. Measured, not reasoned: a host-free mission at 30 steps has `agent` unset on the leader and set on both squadmates. The difference matters to J8, because a seat that has not sent its first packet yet is a soldier standing still, not one the companion brain is covering for |

**The `hosted` flag is derived, never passed.** `this.hosted = !!canvas`. A
constructor argument saying "I am headless" beside a canvas that says otherwise
is two sources for one fact, and the one that would be believed is the argument.

## Three shapes of aim, and one map (J7 — built)

Two changes, and the second one is the one with a design in it.

**The map.** `this.inputs` is owner → anything answering `isDown` /
`justPressed` / `aimSource`, beside `this.control` and `this.ends` as the third
per-commander map. `_updateSoldiers` builds `pilots` (leader → its input) and
drives whoever is in it; `_handleControl` folded its manual swap into the loop
that was already walking every commander. **A commander with no input is not a
branch** — they are absent from `pilots` and fall to the companion brain, which
is one mechanism for three situations: a seat nobody has filled, a commander who
walked away, and (before J8) every seat but this one.

| Verb | |
|---|---|
| `inputFor(owner)` | The input driving `owner`, or null |
| `setInput(owner, input)` | Hand one over; `null` takes it away |
| `sampleInputs()` | J4's one-sample-per-step contract, made plural. `_frame` calls it; a headless driver calls it |

**`this.input` stayed, and is the FALLBACK for `this.owner` rather than an entry
in the map.** That is deliberate and it is not tidiness: `m.input = …` after
`start()` is how three suites drive a mission, and an entry captured at start
would ignore them silently — the assignment would appear to work and change
nothing. As a fallback it also says the true thing, which is that `this.input`
is the device at *this* keyboard and belongs to whoever is sitting at it.

**The aim.** `_applyAim` resolved every source against `this.camera` and
`this._zoom()`. A room holds one scene for two viewers and `this.camera` can
only ever be one of them, so the conversion had to leave. It did not become
"the mission never resolves a camera" — it became three source shapes, split by
who owns a camera:

| Shape | Coordinates | Who reports it |
|---|---|---|
| `stick` | A direction | Any input. Needs no camera and never did |
| `mouse` | Canvas pixels | **The local device only.** Resolved against `this.camera` exactly as before, because for a browser mission the mission IS the viewer |
| `world` | An absolute world point | Anything arriving from elsewhere (J8). Already resolved by whoever owned the camera it came from |

So the browser's path is unchanged down to the arithmetic, and the wire's path
has no camera in it. `test/mission-ownership.test.mjs` asserts both halves and
the contrast between them: a world aim does not move when the camera scrolls
4000px, and a mouse aim does — the second is the control, and it is what says
why `mouse` cannot be the shape J8 sends.

**One thing deliberately NOT per commander: the debug overlays.** `debugGraph`
and `debugPath` stay on `this.input`. An overlay is what the person looking at
this canvas wants to see, not something a commander owns, and a room's mission
draws nothing.

## What a snapshot is, and what it is not (J8 — built)

Three shapes crossed, and the interesting one is the middle.

| | Where | What |
|---|---|---|
| **Two channels, split by cadence** | `server.mjs` | `/api/*` stays exactly the V1–V3 campaign service — a click, an answer, a seat snapshot, a dispatch — and gained ONE event, `missionEnd`. `/mission` is a WebSocket that lives as long as one mission and carries nothing but 60Hz input up and snapshots down. Neither learns the other's rate, which is the boundary "Must not touch" names |
| **A `hosted` flag is the whole of the page's fork** | `src/net/rooms.js` → `src/main.js` | A room that opened a flight marks each of that round's dispatches `hosted` before pushing it. With it the page opens a socket and plays viewer; without it — single-player, hot-seat, and a room whose host declined — `playNext` is the function it always was. There is no second code path for "am I online" |
| **The room does not build the mission, and the loop is not in the registry** | `src/net/rooms.js`, `server.mjs` | `createRooms({ startMission })`. The registry groups the round by `mission.id`, builds the flight, and asks the host to hold it; a host that returns nothing leaves the round exactly as V1 routed it. `test/session.test.mjs`'s source scan on `rooms.js` stays true because a `Mission` never enters this file |

**A FLIGHT IS PER MISSION, NOT PER DISPATCH, and a solo one takes the same
path.** Two commanders on one lead are one level and one simulation — the whole
phase — so the round's dispatches are grouped by `mission.id` and each group is
one flight with one squad, concatenated in round order so `loadMission` opens its
two-slot gap in the same place at both ends. A lone commander in a room is a group
of one and is held identically: "the page keeps simulating when it is alone" would
...
| | |
|---|---|
| **Soldiers are keyed by id; roots and loot are positional** | Approximation 11c, applied. A splice renumbers `scene.soldiers` mid-mission and an index-keyed wire would teleport everybody left behind at the moment somebody walks out. `specRoots` never renumbers (a dead root stays in the array) and `loot` is only appended, so both are legitimately positional |
| **A seat IS sent the other commander's squad** | The one rule here that is additive. `design/multiplayer.md` says another commander's squad is visible on a level you are both standing on. What crosses is a colour, an owner, a health ceiling and the draw fields — **not the name, not the weapon, not the roster record**: the HUD's squad cards are this commander's own and a foreign body never reaches them, so there is nothing on the object to leak |
...
below it is the same, because a camera is a fact about who is looking and the room
holds one scene for two viewers. What is skipped is the eight gameplay calls
between them. The spec's own warning that **"keep rendering, stop updating" does
not work as stated** is what this shape answers.

**The one thing that moved BACK into `_handleControl`'s caller**: the debug
overlay toggles. They were inside `_handleControl`, which a viewer skips, so a
room page would have had `config.debugOverlays` silently dead. They are now
`_handleOverlays()`, called above the early return — an overlay is what the
person looking at a canvas wants to see, which is as true of a viewer as of a
player.

**The banner's squad cards are frozen BEFORE the splice, and this is the one
ordering bug the wire creates that the local mission does not have.** J2's end
record froze the roster as the squad left, reading a live array that still had
them. Over a wire, the snapshot that carries the outcome is the SAME snapshot
that removes the bodies — so the freeze has to happen inside `applySnapshot`,
before the removal pass, or the banner draws over an empty squad list for its
whole second and a half.

**A viewer files no result, and that is what makes "the room sends both
`missionResult`s" true.** `_finish` returns early on a remote mission: the banner
is the whole of what its timer owns. The room computes the result, sends the
command **as that seat**, and pushes the command's ANSWER — the day summary,
which no page could otherwise have seen, because no page sent the command — down
the seat's own turn-boundary stream as `missionEnd`. That push is also what takes
the screen off the canvas.

**What a socket close does, and what it deliberately does not.** The held keys go
with the socket: a commander whose tab closed would otherwise sprint into the
nearest wall for the rest of the mission. It does NOT hand their squad to the AI
— that is approximation 6, it is still one line, and it is still not built,
because "your squad fights on without you" is a design decision and not a
disconnect handler.
...
| `src/mission/mission.js` | J1 and J2. The eleven sites in Background, per-owner `_resolve`, the extraction splice, `this.control` keyed by soldier id rather than array index, and the HUD. **Plus one line J5 owes it:** `start()` verifies that the commander it was handed owns somebody on this level before believing it — see the `src/main.js` row |
...
| `src/net/rooms.js` | J5 and J8. **As built: J5 changed no line of this file**, which the Reuses table predicted in as many words — "J5 changes what a seat is sent, not how". The pairing rides on the dispatch, `routeRound` still keys on `playerId`, and each seat still gets its own dispatch and no other's; `test/service.test.mjs` drives two seats onto one lead to say so. A room holding the pairing itself has no reader until J8, which can group `room.announced` by `mission.id` when it needs to. J8 is still all of the row below. A room gains a mission in flight beside its campaign: which seats are in it, and the per-seat snapshot it hands out. **It does not hold the loop and does not construct the `Mission`** — `test/session.test.mjs` source-scans this file for DOM globals and `localStorage`, and that assertion stays true. **As built (J8): ~90 lines and one new constructor argument.** `createRooms({ startMission })` — a host OFFERS to hold missions and a registry with no host is byte-for-byte V1, which is what keeps `test/service.test.mjs` and every other builder of a registry unchanged. `routeRound` groups the round by `mission.id` into a FLIGHT (seats, dispatch ids, one concatenated squad), asks the host to hold it, and marks that group's dispatches `hosted` before pushing them. The flight carries its own `report(owner, result)` closure — the room files the missionResult through its own `command` path, one per dispatch id, and pushes the answer to that seat as `missionEnd`. `flightFor(token)` is the whole of the mission socket's authorisation, and it is the same rule the three /api routes use: a token is a seat |
| `server.mjs` | J8, **built**. The socket, the room's `Mission`, and the deadline-corrected fixed-step loop that drives it. `netproto/server.mjs`'s loop was copied as specified — catch up at most 5 steps, then resync rather than teleport — and so was its byte readout, which is what approximation 11 asked for. **As built, two things the row did not say.** `startMission` builds the mission with a `null` canvas AND a `null` owner: the room is at no keyboard, so "the commander at this one" has no honest answer, and every commander gets a wire input so the local-device fallback that argument selects is never reached. And the loop walks EVERY live flight per step rather than one — a process holds a room's missions, not a match. **Three things netproto's loop does not have, all of them consequences of this being a file server and a room registry rather than one arena.** It sleeps at 50ms while nothing is flying, because "no mission" is this process's normal state and single-player needs none at all. Each flight's step is wrapped, because an uncaught throw would end the loop for every OTHER room in the process and a room is the only copy of its campaign — one frozen canvas beats every campaign stopping. And `broadcast` skips a commander who has already reported and closes their socket, because "a seat that resolves goes back to base and stops receiving snapshots" is the room's rule and must not be left to a browser closing its own connection |
| `src/net/ws.mjs` (new) | J8, **built**, and it is a **copy** of `netproto/ws.mjs` rather than an import of it. That folder's whole value is that it is standalone and deletable; making the shipping server depend on a measurement prototype would spend that. 200 lines of RFC 6455, `setNoDelay` included — which is the line that matters, because a mission's input packets are exactly the small writes Nagle would coalesce into 40ms of latency that has nothing to do with the architecture. `.mjs`, like `server.mjs`, because it names `node:crypto` and no browser can load it |
| `src/game/config.js` | J8, **built**. One knob, `missionSnapshotHz` (default 20, netproto's measured default). It is the one number this slice introduced and the convention says a number goes in the SCHEMA — its help text used to carry the caveat that a room reads built-in defaults and nobody can turn it; `tech/server-settings.md` C2 made that false and the text says so |
| `src/mission/mission.js`, `src/mission/input.js` | J6 and J7, **both built**. A host-free construction path (no `input.enable`, no rAF), and input that arrives per owner rather than from one device. `_applyAim` takes aim already in **world** coordinates, because a server holding one scene cannot resolve two viewers' cameras. **As built (J6): 35 lines across the two files and not one of them in `update()`** — an optional canvas plus a derived `hosted`, a start() that arms the device and the loop only when hosted, a `render()` that returns without a context, `MissionInput.reset()` split out of `enable()`, and `disable()` guarded on `_enabled`. Everything else J6 needed already worked in bare node. See "What a host actually is". **As built (J7): `src/mission/input.js` was not touched at all.** The row assigned J7 to both files and it needed only one: a `MissionInput` is already a thing with three read names on it, and per-owner input is a question about who HOLDS one, not about what one is. See "Three shapes of aim, and one map" |
| `src/net/mission-socket.js` (new) | J8, browser half, **built** — and it is 90 lines, because the two hard parts were already solved. Opens the socket, sends one packet per FIXED STEP (J4's contract, not the frame rate), drops a snapshot older than the one on screen, and resolves this page's mouse aim to WORLD coordinates before it crosses (J7's three shapes). Beside `src/net/remote.js` rather than inside it: that file is the turn-boundary client and its three names are pinned. **As built it has two verbs, not one** — `step(mission)` and `listen(mission)` — because the socket is opened BEFORE `Mission.start`, which takes it as an argument, and a snapshot arriving in that gap has no scene to land on |
...
| `src/main.js` | J2, J5 and J8. **J5's share is one line**: J2 named the seat only when the squad actually declared owners, which was a guard against a projection that did not emit them; now that it does, the guard is a tautology and `mission.start` takes `current.playerId` unconditionally. **Corrected after J5 was played: deleting the guard was wrong, and it MOVED rather than went.** "The projection emits owners" is a fact about the version that BUILT a dispatch, not about the page playing it — a page is static files and reloads, a room's campaign lives in the `server.mjs` process and does not, so the two halves are legitimately of different versions for as long as that process runs. Against a pre-J5 campaign the page then named a commander who owned nobody, `piloted` in `_updateSoldiers` resolved to `undefined`, and EVERY soldier fell to the companion brain — the keyboard's included, silently. The check now lives in `Mission.start`, where the scene is rather than where the squad argument is, and it is narrow: a scene that declares NO owners takes this seat's name off (there is one commander on it and the page was handed the dispatch), while a scene that declares owners and not this one keeps it and pilots nobody, because handing this keyboard another commander's squad is a worse answer than handing it none | Per-owner `onComplete`, and **the `owner` argument `mission.start` is never passed** — `start(current.mission, current.level, current.squad)` has no fourth argument, so J1's default makes the local commander whoever spawned first, which is wrong for one of two seats (J2). **As built (J8): ~25 lines and one fork.** `playNext` opens a `createMissionSocket` when `current.hosted` and hands it to `mission.start` as its fifth argument; `showPushedResults` is the room's half of `onMissionComplete` and is installed off `transport.onMissionEnd` beside `onDispatch`. `onMissionComplete` itself is untouched, and is still what single-player and hot-seat use. **What did NOT need building: joint missions starting together.** Both dispatches are announced by one `closeRound` and the room opens the flight in that same call, so one simulation starts once for both seats whatever either browser is doing — the queue this row worried about was a property of the page being the host |
| `src/net/mission-wire.js` (new) | J8. The input packet and the snapshot — the one place a scene is narrowed for the wire, **per recipient**. **What a snapshot MUST carry is the other commander's squad**: `design/multiplayer.md` says their squad is "visible only on a level you are both standing on", and a joint level is that place. Position, facing, stance, health and aim — what a soldier is drawn from. Not their loot, not their result. Stated because every other rule here is subtractive, and "per recipient" read as "your own squad only" would pass all of them while deleting the feature. `netproto` sends everyone the same snapshot because a one-screen arena has nothing to hide; this game does: `design/multiplayer.md` never discloses what another commander recovered, and `sampleScene` in `src/mission/checksum.js` — the honest starting list — carries exactly that. **As built: it does carry the other squad, and `collected` crosses as a COUNT for BOTH commanders rather than as items for one.** The campaign gets the items from the room's own `_resolve`, and the HUD only ever printed a number, so nothing needed the list. What a foreign soldier carries is a colour, an owner, a health ceiling and the draw fields — no name, no weapon, no roster record — which is less than the row asked for and is the same feature. See "What a snapshot is, and what it is not" |
| `src/mission/render.js`, `src/mission/mission.js` | J8, browser half. A page in a joint mission draws a snapshot. **"Keep rendering, stop updating" does not work as stated**: `_updateCamera` is the last call in `update()`, and `this.time`, the particles, the shake, the damage flash and the intro timer all advance there while `render()` reads them. So a viewing page still steps its cosmetic and camera state; what it stops doing is simulating gameplay, and the snapshot replaces the fields `sampleScene` names rather than the whole scene. **As built: `src/mission/render.js` was not touched at all, and `mission.js` took ~45 lines.** The renderer needed nothing because the client MIRRORS the scene into the same objects rather than drawing a wire format — it builds the identical scene from the same level and seed, and every drawn field is overwritten in place. `mission.js` got a fifth `start` argument (the net driver, from which `remote` is derived the way `hosted` is derived from the canvas), the early return in `update()`, an early return in `_finish`, `toWorld`, `lootCount`, and `_handleOverlays` split out of `_handleControl` so a viewer's debug keys are not silently dead |
| `test/mission-ownership.test.mjs` | **As built (J7): 103.** The 24 newest are the whole of the per-owner input axis — two commanders each moving their own leader and nobody else's in one scene, a swap read off the pressing commander's own input, a commander whose input is taken away falling to the companion brain mid-mission, `sampleInputs()` reaching every input exactly once (a driver that samples one leaves the other seat on a frozen frame, which looks like lag rather than like a crash), and the aim block: a world point that does not move with the camera, a mouse point that does — the control that says why `mouse` cannot be the shape that crosses a wire — and two commanders aiming opposite ways in one step. Verified by mutation rather than by writing them green: reverting the pilot lookup, the swap read and the world branch reddens 4, 2 and 2 by name. **As built (post-J5): 79.** The 7 newest are the version-skew case above — a named commander who owns nobody, which is the one way this axis fails totally and in silence, and which reached a live room rather than the bar. **Exists, committed with J1**, 44 assertions covering the axis, the line-up, control that cannot cross, the dead-leader swap, escort anchoring, loot credit, per-owner `_resolve`, and the team/owner boundary. **As built (J2): 72**, and the 28 it gained are the whole of the second half — an exit that ends one mission and not the other, a level that keeps moving for whoever is left, an extracted squad that leaves the array while nobody else's control shifts, a keyboard whose commander has gone home reaching nobody, a resolved owner `_checkOutcome` skips rather than reading as wiped, the losing branch J1 narrowed, `onComplete` firing once per commander over 200 frames rather than 104 times, the artifact going to whoever extracted first, and a result frozen against the round still in the air |
...
|---|---|
| `test/mission-golden.test.mjs` | **The load-bearing one.** A mission replays from its seed at a fixed step, at one owner. A re-baselined golden here is the bug, not the fix. Two things it did NOT guard, which is why it is not sufficient: it drives `m.update(STEP)` directly and never runs `_frame`, so **J4 is invisible to it**; and its 41 samples never resolve, so it says nothing about `_checkOutcome` or `_resolve`. **As built (J4): the first of those is no longer true.** Section (4) drives the real `_frame` off a synthetic clock at 20, 30, 60 and 144fps and asserts all four play the mission a bare `sample()`/`update(STEP)` loop plays. The golden's own 41 samples and its baseline did not move — the new block builds its own missions and shares the file for the trace, the level and the sampler. It also asserts what J6 and J8 will stand on: **a mission stepped with no frames at all is the same mission**. **As built (J6): 17 → 28**, and J6's half is section (5), in two pieces because the question has two halves. In-process: a canvas-less mission is sample-for-sample the mission a canvas-ful one plays, its viewport is the config preset, `render()` is a no-op and `stop()` releases an input that never bound a device. Out of process: a **child node process with no harness at all** runs the same trace and is compared to this one. The child is not tidiness — `test/run.mjs` installs the DOM before any suite, so deleting globals in here would only prove this process can be made to look bare, never that the module graph did not reach a host on the way IN, which is an import-time question. The child reports rather than throws, and it asserts that none of the four globals exist before it starts, so it cannot pass for the wrong reason if node ever ships one |
...
**should** be, because by then it is a subsystem rather than a probe. **As built:
it is** — section 4 of `test/mission-net.test.mjs`, spawning the real
`server.mjs`, and it costs about two seconds. Two things it took to get there
that the plan did not name: node has no `EventSource`, so the suite reads SSE
off `fetch`'s body stream in ten lines; and `config.leadVisibility` is 0.5 in the
SERVER's process where no test can reach it, so whether two commanders share a
lead is a coin flip — the answer is to open rooms until one lands rather than to
stub a config the room is not reading. **What it still cannot see is `src/main.js`**,
which is the last unguarded file in the phase and is guarded by playing.

**J6 added the instrument that makes "the bar cannot see a bare process" false**:
a suite may spawn one. `test/mission-golden.test.mjs` section (5) runs
`process.execPath` with `--input-type=module -e`, no harness, and compares the
child's samples to its own. It costs one spawn, it is in `node test/run.mjs`,
and it is the only way to ask an import-time question about a module graph this
process has already imported. **J8 can reach `server.mjs` the same way** — that
file binds a port on load, which is exactly the reason no suite imports it and
exactly what a child process makes harmless.

## Approximations

| # | Where it is not exact | What catches the failure |
|---|---|---|
| 1 | **Two humans cannot drive two squads on one machine, so J1 and J2 are not playable before J8.** The Mission holds one `MissionInput` and one `controlled`, and `src/main.js` disables seat swapping mid-mission. Hot-seat gives one commander plus AI escorts wearing another owner's colours — useful for looking at, worthless as proof of feel. The headless suites are the real guard, and the first time a joint mission is *played* is J8. **Spent: J8 is built, and the way to play one is two browsers.** `node server.mjs`, open `index.html?room=2`, follow one seat link on each machine (or in two windows), deploy both commanders onto a lead they can both see, and ready up. The half of this that survives is that hot-seat still cannot do it and never will | Nothing. What replaces it is that the feel of the thing — approximation 2b's round trip — is now measurable and has never been measured on this game |
...
| 5 | **A joint mission runs on the SERVER's config, and nobody's browser settings apply — the knobs stay knobs, but nothing can turn them.** Friendly fire is still the toggle `design/multiplayer.md` specifies and still defaults off; what a room lacks is any path to SET it, because the server has no `localStorage` and so reads built-in defaults. `config.friendlyFire`, `gravity`, `aimSpread` and fourteen other knobs are localStorage, read live through the mission's `_ctx` getters. In the room they resolve to built-in defaults — `tech/multiplayer-service.md` approximation 11 by another route, and now it reaches gameplay feel rather than just the campaign. It is no longer a divergence risk, because there is one reader; it is a **surprise**: a solo mission and a joint mission on the same machine obey different numbers | **Closed by `tech/server-settings.md` (C1–C3).** `scope: "server"` names the 47 knobs a room owns, `GET`/`POST /api/config` carry them, and `editor.html?server=1` tunes a running server. What remains of this row is its shape and not its sting: the room's values are still the room's, they still die with the process, and a solo mission on the same machine still obeys the browser's numbers — but there is now a path to SET them, which is the whole of what was missing |
| 6 | **A commander who leaves mid-mission is nearly free now, and is still not built.** The server holds the only simulation and `updateCompanionSpec` already drives every unpiloted soldier, so the design's "handed to the AI and fights on as companions" is a socket close feeding one flag. `tech/multiplayer-service.md` approximation 10's deadlock does NOT survive this architecture — the mission runs on regardless of who is watching — but the flag and the result routing are unwritten. **Still true after J8, and the socket close is now a real place to put it: `server.mjs`'s `conn.onclose` RESETS that seat's held keys and does nothing else.** Their soldier stands still rather than sprinting into the nearest wall for the rest of the mission — which is the smallest honest thing, not the design's answer. `mission.setInput(owner, null)` is the one line that would hand the squad to the brain, and it is deliberately not written, because "your squad fights on without you" is a design decision and a disconnect handler is not where one gets made | Nothing. Recorded because the blocker shrank three times: from "needs a second simulator" to "needs a signal", to "needs a signal the socket already gives us", to "needs somebody to decide what should happen" |
...
| 11a | **A snapshot must be filtered per recipient, and the model it is copied from is not.** `netproto` broadcasts one payload to everybody because a single-screen arena has nothing to hide. Here `design/multiplayer.md` says what another commander recovered is never disclosed, and the honest starting list (`sampleScene`) carries the collected count and each drop's collected flag. The per-recipient shape exists in `netproto` for one field only — `ack` | **Built and asserted, and the answer turned out narrower than the worry.** A snapshot differs between two seats in exactly three fields: `a` (their own ack), `c` (their own loot count) and `e`/`k` (their own outcome and the soldier they are driving). Everything else is the level they are both standing on. What is NOT on the wire for anybody is an item — the campaign gets those from the room's `_resolve`, so the count is all the HUD ever needed. `test/mission-net.test.mjs` section 2 asserts both hauls are absent from the serialised snapshot by name |
...
| 11 | **The mission's live scene is bigger than a snapshot, and what gets sent is a judgement call.** `netproto` sends positions rounded to a tenth of a pixel and never sends velocity, deliberately, so a client cannot half-predict. This mission has spec roots with nested parts, brains, statuses and effects — `sampleScene` in `src/mission/checksum.js` is the starting list, but rendering needs fields a checksum does not | **Measured: 570 bytes on a Low lead, ~11 KB/s per seat at 20Hz.** `server.mjs` prints B/s per flight every five seconds and the suite asserts a 16KB ceiling per snapshot, so a widening that would work on a LAN and fail on a wire is visible in both places. Velocity is deliberately NOT sent, following netproto: a client that cannot half-predict cannot half-predict wrongly. What DID have to be added past `sampleScene` — the telegraph pulse, hit and muzzle flashes, stance, aim, and the def id of an entity the simulation created mid-mission — is all rendering, and is listed under "What a snapshot is, and what it is not" |
