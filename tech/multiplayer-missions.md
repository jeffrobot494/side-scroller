...
input decoupled from a frame rate for the same reason a peer did. **J4, J5 and
J6 are built too**, which leaves J7 and J8: input per owner, and the room that
holds the mission. **Nothing plays jointly until J8**, and every slice before it
lands with no transport at all.
...
| **Input is not "injected" yet, and does not have to be** | The slice row says "input injected rather than device-bound". As built a host-free mission reads blank frames off its unenabled `MissionInput`, so it runs with *no* input rather than requiring one — enemies fight, the AI squad plays, nobody is piloted. That is the correct floor: J7 is what gives a mission somewhere to inject input per owner, and J6 should not invent a narrower seam that J7 then replaces |
