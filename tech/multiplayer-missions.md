...
input decoupled from a frame rate for the same reason a peer did. **J4, J5, J6
and J7 are built too**, which leaves **J8 alone**: the room that holds the
mission. Everything the scene needed is in — an owner axis, independent ends,
two results for one lead, fixed-step input, a joint dispatch, a host-free
construction path, and an input per commander. **Nothing plays jointly until
J8**, and every slice before it landed with no transport at all.
