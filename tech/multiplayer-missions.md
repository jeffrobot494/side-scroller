...
| `src/game/config.js` | J8, **built**. One knob, `missionSnapshotHz` (default 20, netproto's measured default). It is the one number this slice introduced and the convention says a number goes in the SCHEMA — with the caveat written into its own help text, that a room reads built-in defaults and so nobody can turn it (approximation 5) |
