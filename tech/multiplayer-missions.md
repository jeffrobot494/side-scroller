...
exactly and J5's the day J5 lands. `onMissionComplete` took the trailing
...
| `src/game/session.js` | J3 (built) and J5. The round learns that two dispatches can name one lead, and says so in the pair it emits. **J3's half is one predicate in the `missionResult` case**: is any other still-outstanding dispatch naming this lead. J5's is the pairing on the projection |
