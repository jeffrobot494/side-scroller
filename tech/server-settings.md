...
| 2 | **Reset stays local and is disabled in server mode; Export and Import are not.** `resetConfig` writes to the browser and its `writeStore({})` is a no-op in node, so over a wire a Reset would refresh the sliders and look like it had worked while the server kept every value. Export and Import both cross: Export because it is the design's only permanence path, Import because a config that can be read out and not put back is a one-way trip. **`importConfig` itself is NOT reused over the wire** — it writes the local object — so the browser half parses and validates against the schema and then posts each key, which is the same write a slider makes. Export emits the browser's values, Import writes the browser's, Reset calls `resetConfig` — whose `writeStore({})` is a no-op in node, so over a wire it would look like it worked. The design's permanence story is Export → paste into source, and in server mode that exports the wrong machine | C3 disables all three when a server is attached, which is smaller than making them work and is honest about which machine you are looking at |
