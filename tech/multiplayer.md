...
| M3 | ~~Lockstep input~~ — **deleted, not re-cut.** There is one simulation, so there is no second input stream to reconcile and no delay window to tune. What replaced it is four slices (J5–J8): the joint dispatch, a mission that runs without a browser, input per owner, and the room's loop | — |
...
| The payoff is still last, and now it is a round trip away | Phase 3's own slices reordered once already when J0 answered. What remains untested by anything but playing is **feel**: `netproto/` measured ~60ms input→pixels on a good path and called it good, but that was a versus-shooter with two rectangles, not this game with a squad, and no prediction is planned. A distant player has a permanently worse game than a near one |
| The alternative won | A **server-authoritative** prototype — client sends inputs, server owns the world, client draws snapshots and predicts nothing — was built standalone in `netproto/` and deployed. Measured: the architecture's own cost is ~33ms at 20Hz snapshots and ~16ms at 60Hz; it plays well at ~35ms RTT and is unplayable at 210ms; and one process owns a match, so it cannot scale horizontally without sticky routing. Findings in `netproto/README.md`. **It is what Phase 3 now builds**, because J0 measured that two V8 versions disagree on four transcendentals — so a lockstep game would have been correct until a Chrome release nobody here controls. `tech/multiplayer-missions.md` carries the decision and the numbers |
