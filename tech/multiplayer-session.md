...
| 2 | **A round-trip check is not a schema.** `JSON.stringify` refuses almost nothing: it drops functions and `undefined` silently and renders a `Map` as `{}`. So the check is a COMPARISON — what came back must match what went out — not a call that throws. This is not hypothetical: `seenBy` is a `Map` on every lead in a multi-commander world, and it is on the raw lead a dispatch carries until W2 projects it | The loopback's own comparison, and `test/transport.test.mjs` |
| 3 | **Async in name only.** Everything resolves on a microtask, so no screen ever renders a pending state and nothing asks what a click looks like while it is in flight | Nothing. It is a design gap, named to Bo, and T2 is where it has to be answered |
...
| T4 | Presence and disconnection — the first slice in this phase that needs design, and the one `design/multiplayer.md` speaks to already (a player who leaves mid-mission hands their squad to the AI) |
