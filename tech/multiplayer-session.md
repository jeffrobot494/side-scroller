...
| **W3** | **The view crosses the seam.** The page stops reading the session's live projection and reads a **snapshot**, refreshed by a push. **The push is a broadcast**: one seat's command routinely moves shared world state and other seats' campaigns — the day, the doom clock, lead expiry and arrivals, the world log every War Room renders, another seat's readiness, and `share`, which is the one command that puts a lead on somebody else's board. So every seat's snapshot is refreshed on every command, each with its own projection and nothing else. **The snapshots live on the transport, not on the client**: `connect` is a free function and a client's surface is pinned to three names, so there is nowhere on a client to keep a registry. The hub and the ambient layer keep reading through a **stable handle** — one campaign-shaped object per seat whose identity never changes and whose every field resolves against the current snapshot on read. `session.view()` is unchanged — the server half still projects live, which is what keeps its suite meaningful | Yes, and not the way the plan said. The hub renders a copy, which costs nothing (no consumer writes through a view). What a player can SEE is that the other commander's readiness, the shared clock and a shared lead now appear without them clicking anything — the design says all three are visible, and today only a click repaints |
...
**Three things W3 has to get right, none of which were in the plan.**

| | |
|---|---|
| **The handle re-reads, it does not cache** | `advanceDay` and `applyMissionResult` **replace** `state.leads` and `state.roster` rather than mutating them (`src/game/state.js`). A handle that copies field values at refresh time is stale one command later even while pointed at the right snapshot. It must be an OBJECT, not an accessor function: `livingRoster(g)` and `soldierMaxHp` are handed it directly by both the hub and the ambient layer |
| **The broadcast fires inside `command`, before the answer** | Every hub write site renders inside its answer callback, and `showResults` renders inside the `missionResult` answer. A broadcast that lands after delivery draws every one of them one command stale — no "squad committed" tag, a results screen printing the previous day and the pre-mission roster, an `outcome` that never promotes the end screen. This is the position W2's round announcement already occupies |
| **It fires on refusals too** | `closeRound` empties every commander's pending list and writes `round.last` BEFORE `endRound` can refuse the day. A broadcast gated on `res.ok` misses a state change that has already happened, and `test/session.test.mjs` already pins that ordering |

**And the second door has to be narrow.** `Hub.setView` nulls nine transient
fields because it is a SEAT SWAP. As the refresh path it would blank the results
screen's dead (`_lastSquad` is the fallback that exists for exactly the moment
`applyMissionResult` has dropped the casualties from the roster), throw away a
refused deploy's picked squad, and clear the in-flight control marker. The
repaint door must leave `mode`, `location`, `flash`, `deploy`, `result`, `turn`,
`sold`, `shareOpen`, `_lastSquad` and `pending` alone.
...
| The DOM-free, storage-free rule **and its assertion** | `src/game/session.js`, `test/session.test.mjs` | The property that lets the session run somewhere else. The new modules inherit the RULE; they do not inherit the guard — the source scan reads `src/game/session.js` and nothing scans `src/net/` |
| The five bindings a seat swap moves | `src/main.js`, `src/hub/hub.js`, `src/hub/ambient.js` | A client per seat re-points the same five things `swapTo` already re-points. The shape is built; what hangs off it changes. `src/hub/hotseat.js` is NOT one of them — it holds a seat id and a `<select>`, never a view. The cross-seat surface that does hold one is the hub's own task-force strip |
...
| `src/hub/ambient.js` | Reads its roster every frame, so it takes the same stable handle the hub does rather than a snapshot object captured once. It must be populated at CONSTRUCTION: `createHubAmbient` steps and draws once synchronously at mount, and the hub's first render dereferences `money` — a handle that is empty until the first broadcast crashes at boot |
| `src/net/client.js` | Its `view()` stops delegating and returns the seat's handle. The surface stays three names — `test/transport.test.mjs` pins `Object.keys` and that `view` takes no argument, so a per-seat callback cannot be hung here |
| `test/transport.test.mjs` | The loopback: delivery order, a payload that does not survive a round-trip refused, one client per seat, a snapshot carrying only its own seat's projection, and a broadcast reaching every client. W2 edited its two `takeRound` assertions and added the announcement, the projection's field list, and the buffer. **W3 rewrites the assertion that pins `client.view() === session.view(playerId)`** — it was put there at W1 to make this slice a deliberate act, and this is that act |
| `test/session.test.mjs` | The `src/main.js` source scan follows the bindings it pins — including the literal `createLoopback((announce) => createSession(` it now matches, which W3 edits in the same commit if a second construction-time channel changes that shape (four call sites in `test/transport.test.mjs` move with it). **W2 also rewrites the identity assertions it cannot keep** — `mission === leadOf(...)` and `squad[0].data === roster[0]` become assertions about the projection, in the same commit, and the comment beside the live-soldier one explains what changed rather than being deleted. **As built:** two, not six. The level and both weapon identities survive untouched — see As built (W2) |
...
| 5 | **The snapshot is whole, and taken per command.** No diffing, no dirty tracking, and every seat is refreshed on every command. The premise that this is wasteful is mostly wrong, which is worth stating so nobody narrows it on that argument: `hire`, `commission` and `sellLoot` all write the WORLD log, which every seat's War Room renders, and only `deploy` and `release` move one seat alone | None needed at this size — a roster, a board, an armoury and a log. Recorded so a later profile knows it was a choice |
| 6 | **Identity is lost at W3 — for seven fields, and it is not the interesting risk.** `leads`, `pending`, `taskForce` and `elsewhere` stopped being the campaign's own objects at S6 and W2; what W3 copies is `roster`, `armory`, `recruits`, `stores`, `building`, `log` and `completedMissions`. Verified safe: no consumer writes through a view — no assignment through the handle, no `push`/`splice`/`sort` on a view array, the deploy screen holds id strings, and `_lastSquad` has been dispatch copies since W2. **The real hazard is liveness, not identity** — see the handle rule above | `test/transport.test.mjs` pins the live-projection assertion today and W3 rewrites it. Beyond that the guard is playing it, which is why W2 and W3 are separate commits |
...
**Reviewed before W3 (fresh context, against the code W2 left behind).** Nine
corrections, all verified in the code before being written down. The four that
would have cost a build are in the Slices section — the handle must re-read
rather than cache, the broadcast fires inside `command` and on refusals too, the
snapshots live on the transport because a client has nowhere to keep them, and
`setView` is not the door. The rest corrected claims this document made about its
